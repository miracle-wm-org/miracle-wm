...
void WindowManagerToolsWindowController::open(miral::Window const& window)
{
    ...
    {
        container->set_animation_alpha(1.0f);
        return;
    }
    ...
    {
        process_animation(AnimationFrameResult { true, rect, std::nullopt, 1.0f }, container);
        return;
    }
...
{
    // The spread focuses windows on hover and on click, so [overview] selects
    // just like [normal] does.
